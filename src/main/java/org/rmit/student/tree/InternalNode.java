...
/**
 * Internal/Index nodes which contain the keys used for indexing and the pointers/children that references
...
public class InternalNode<K extends Comparable<K>> extends Node<K> {
    ...
    /**
     * Gets the index to insert the new key and then adds it. Checks if an overflow occurred and calls the function
     ...
     *
     * 3. The keys of the current Internal node will be split between itself and the new node
     *
     * 4. Set the parent for the new Internal node to be the same as the current Internal node
     *
     * 5. Update the children/pointers for the parent node. The current (left) Internal node will maintain its current
     * position in the parents pointers/children list while the new node will be added next to it. Every other node
     * after this will be pushed back
     *
     * NOTE: Since we are using ArrayList instead of Vector the other entries in the array list are not pushed back,
     * since ArrayList uses doubly linked list internally
     *
     * 6. Insert/Push the middle key (the first key in the right/new node) up to the parent
     */
    @Override
    public void handleOverflow() {
        ...
        // The + 1 here is because in Internal nodes the first key in the right/new node is pushed up to the parent
