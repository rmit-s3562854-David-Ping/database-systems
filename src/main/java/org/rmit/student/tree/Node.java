...
public abstract class Node<K extends Comparable<K>> {
    ...
    /**
     * Utilizes the binarySearch method of the Collections framework to quickly find the insertion point for new keys
     * The binary search upon failing (which it will do) will return the position where the key would have been inserted
     *
     * We do need to process it slightly though (+ 1 to the result then get the absolute of it) to find the exact index
     */
    public int getInsertionIndex(K key) {
        // Use binary search to find the insertion index instead of traversing through all the keys one by one
